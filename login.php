...
if (isset($_POST['form_sent']) && $action == 'in')
{
	...
	$authURL=sprintf("https://%s:%s@www.openstreetmap.org/api/0.6/user/details",urlencode($form_username),urlencode($form_password));
		...
		if ($sc_pos > 0) {
			if ($sc_pos > 0) {
				...
				if (strlen($username) == 0)
					message('A problem was encountered: The display name cannot be empty. Please setup your <a href="https://forum.openstreetmap.org/viewtopic.php?pid=1087">\'Display name\'</a> which you can find on the \'My Settings\' page on the OpenStreetMap main page.');
			}
			else
	...
	else
	{
		if ($header['status'] == 'HTTP/1.0 500 Internal Server Error') {
			...
			//message('A problem was encountered: The OpenStreetMap authentication API is currently not working, so unfortunately you cannot login. Please try again or come back later.');
			message("Authentication failed. Please check if login works on <a href='https://www.openstreetmap.org'>www.openstreetmap.org</a>. If you can't login there, then there's no point in trying here....");
		}
		$authorized = false;
	}
